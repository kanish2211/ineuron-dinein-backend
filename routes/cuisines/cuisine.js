...
router.post('/',auth,async(req,res,next) => {
    ...
    try{
        ...
        if(!isAdmin)
            return res.status(401).json({error: "You are not authorized to add cuisines"});
        const isCuisineAvailable=await Cuisine.findOne({cuisineName});
        if(isCuisineAvailable) 
            return res.status(400).json({error: `Cuisine with this name ${cuisineName} exists`});
        await Cuisine.create(new Cuisine({cuisineId:shortId.generate(),cuisineName}));
        return res.status(200).json({message: `${cuisineName} cuisine has been added successfully`});
    }catch(error) {
